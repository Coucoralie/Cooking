...
mongoose.connect("mongodb+srv://testWeb:test@cluster0.rkyfx.mongodb.net/cooking?retryWrites=true&w=majority",{
    ...
    User.register(newUser,req.body.password,function(err,user){
        if(err){
            ...
            passport.authenticate("local")(req,res,function(){
                req.flash("success","Congratulations and Welcome!!!");
                res.render("signup");
            });    
        }
    });
    ...
    User.findOne({username : req.body.username},function(err,userFound){
        if(err){
            ...
            const transporter = nodemailer.createTransport({
               service : 'gmail',
                auth : {
                    user : 'cookingcoralie@gmail.com',
                    pass : process.env.PWD
                }
            });
            const mailOptions = {
                from : 'cookingcoralie@gmail.com',
          ...
          resetPasswordExpires : {$gt: Date.now()}
            ...
            if(err){
                ...
                if(req.body.password==req.body.password2){
                    User.findOne(
                        {username : obj.username},function(err,user){
                            if(err){
                                console.log(err);
                            }else{
                                user.setPassword(req.body.password,function(err){
                                       if(err){
                                            ...
                                            Reset.findOneAndUpdate({resetPasswordToke :req.params.token},updatedReset,function(err,obj1){
                                                if(err){
                                                    console.log(err);
                                                }else{
                                                    res.render("/login");
...
app.put("/dashboard/myreceipes/:id/:ingredientid",isLoggedIn,function(req,res){
    ...
    Ingredient.findByIdAndUpdate({_id: req.params.ingredientid},ingredient_updated,function(err,updatedIngredient){
        if(err){
            ...
            req.flash("success","Your ingredient has been updated!");
            res.redirect("/dashboard/myreceipes/"+req.params.id);
        }
